...
//    private static int scoreAI = 0;
    ...
    private void update(MouseEvent e) {
        ...
        if (field[cellY][cellX] != DOT_EMPTY) return;
    ...
    private void showMessageGameOver(Graphics g) {
        ...
        g.fillRect(0,220, getWidth(), 100);
        g.setColor(Color.YELLOW);
        g.setFont(new Font("TimesNewRoman", Font.BOLD, 60));
        switch (stateGameOver) {
            case STATE_DRAW:
                g.drawString("DRAW", 200, getHeight() / 2);
                break;
            case STATE_WIN_HUMAN:
                g.drawString("HUMAN Wins!", 115, getHeight()/ 2);
                break;
            case STATE_WIN_AI:
                g.drawString("AI Wins!", 200, getHeight()/ 2);
                break;
        }
    }
